...
class Auth {
    ...
    public function getPublicRoles() {
        ...
        // Also exclude suspended roles
        $stmt = $this->pdo->prepare("SELECT role_name, role_key FROM sys_roles WHERE role_key != 'super_admin' AND is_system_role != 1 AND is_suspended = 0");
        $stmt->execute();
